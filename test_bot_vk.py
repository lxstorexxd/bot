...
def get_film(id_user,min_years, max_years):
    ...
    for req in response:
        ...
        number_film = soup.find_all('div', id='div_mustsee_main')
        for objid in number_film:
            id_film = objid.find_all('div', objid=True)
            for id_film_for_url in id_film:
                url_poster = "https:" + f"//st.kp.yandex.net/images/film_iphone/iphone360_{id_film_for_url['objid']}.jpg"

                p = requests.get(str(url_poster))
                out = open("photo_film.jpg", "wb")
                out.write(p.content)

                upload_to_server = vk.method('photos.getMessagesUploadServer')
                req_to_server = requests.post(upload_to_server['upload_url'], files={'photo': open('photo_film.jpg', 'rb')}).json()
                link_photo = vk.method('photos.saveMessagesPhoto', {'photo': req_to_server['photo'], 'server': req_to_server['server'], 'hash': req_to_server['hash']})[0]
                attachment = "photo{}_{}".format(link_photo["owner_id"], link_photo["id"])
                vk.method('messages.send', {'user_id': id_user, 'message': common_name + '\n\n' + common_score + '\n\n' + descriptions, 'attachment': attachment, 'random_id': rd(1, 1000000)})
            ...
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if message == '↓ скрыть ↓':
                    ...
                    vk.method('messages.send',
                        ...
                        get_film(id_user=id, min_years=1920, max_years=2021)
                    ...
                    get_film(id_user=id, min_years=1920, max_years=2000)

                elif message == '2001 - 2010':
                    get_film(id_user=id, min_years=2001, max_years=2010)

                elif message == '2011 - 2021':
                    get_film(id_user=id, min_years=2011, max_years=2021)

                elif message == 'новинки':
                    get_film(id_user=id, min_years=2021, max_years=2021)
    except:
        pass
