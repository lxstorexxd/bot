...
def schedule():
    tomorrow_date = (datetime.datetime.now() + datetime.timedelta(days=1)).strftime("%d.%m")
    upload = vk_api.VkUpload(vk)
    photo = upload.photo_messages('npo.pdf_dir/0_npo.pdf.jpg')
    owner_id = photo[0]['owner_id']
    photo_id = photo[0]['id']
    access_key = photo[0]['access_key']
    attachment = f'photo{owner_id}_{photo_id}_{access_key}'
    vk.method("messages.send", {"chat_id": event.chat_id, "message": f"Расписание на {tomorrow_date}", "attachment": attachment, "random_id": 0})
...
def wiki(response):
    try:
        ...
        search = wikipedia.summary(response, sentences=3)
        write_msg(event.chat_id, search)
    except:
        write_msg(event.chat_id, "Запрос не найден")


dictionary = {'опд': '[id331853910|Ковшикова Юлия Сергеевна]',
              'математика': '[id1084456|Кисова Любовь Ивановна]',
              ...
              'астро': '[id5537426|Дусанюк Алина Валерьевна]'
              }
request = []
    ...
    try:
        for event in longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW and event.from_chat:
                ...
                if request in dictionary:
                ...
                elif 'эдик, что делаешь' in request or 'что делаешь' in request:
                ...
                elif request == 'эдик, погода' or request == 'погода' or request == '/w':
                    ...
                    write_msg(event.chat_id, "pong")
                elif request == 'расписание' or request == '/2':
                    schedule()
                elif '/' in request and request != '/2' and request != '/w':
                    responce = request[0:]
                    wiki(responce)
    except:
        pass